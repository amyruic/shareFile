...
uint64_t dep_read() {
    uint64_t index=0;
    uint64_t sum=0;
    for(uint64_t i=0;i<(uint64_t)ARRSIZE*ITERATIONS;i++) {
        uint64_t arrayval=array[index];
        sum+=arrayval;
        index=(index+arrayval-1+STRIDE)%ARRSIZE;
    }
    return sum;
}
...
int main(int argc, char** argv) {
 array=mmap((void*)0,ARRSIZE,PROT_WRITE|PROT_READ,MAP_SHARED|MAP_ANONYMOUS,0,0);
    ...
    printf("%d elements, %lf cycles/element\n",ARRSIZE,2*time/(double)ARRSIZE/ITERATIONS);
